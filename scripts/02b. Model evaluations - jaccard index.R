...
# On crée un array dans lequel seront stockés les évaluations Jaccard de
# tous les modèles
allsp_jaccard <- array(
...
# Et on prépare une liste qui va contenir le détail des calculs au cas où on 
# veut y revenir plus tard
jaccard_list <- list()
...
for (i in 1:nrow(sp_list))
{
  ...
  calib_lines <- get_calib_lines(model_runs)
  # Récupération des prédictions des modèles
    ...
    for (cv in unique(model_preds$run))
    {
      ...
      for (md in unique(model_preds$algo))
      {
        ...
        cur_preds <- model_preds[
          which(model_preds$points %in% cur_eval & # Lignes d'évaluation 
