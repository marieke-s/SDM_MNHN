...
for (i in 1:nrow(sp_list))
{
  ...
  em_runs <- BIOMOD_EnsembleModeling(
    ...
    em.algo = c("EMmean", "EMmedian", "EMcv", "EMci", "EMca", "EMwmean"), # Quelles méthodes pour le modèle d'ensemble ? # EMwmean moyenne pondérée par l'évaluation = non conseillé car sous-tend qu'on a suffisamment confiance dans notre métrique d'évaluation. # EMca : somme par pixel et on décide si selon une règle de démocration on juge présence ou abscence (résultat binaire 0 ou 1) -> très rarement utilisé, EMci = donne l'intervalle de confiance (sort 2 carte ci Inf ate ci Max, pour les 2 bornes).
