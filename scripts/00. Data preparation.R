...
for(i in 1:nrow(proj_names)) {
  ...
  # Check how long it took to run the loop
  cat(paste0(Sys.time(), " - ", final_name, "  done\n"))
  
}

# Recommendation : quand on lance un modèle c'est mieux de brancher l'ordi, car sur batterie, le processeur est bridé. 
...
sp_list <- read.csv("data_cours/species_list.csv", sep = ";") # parametres definissant la programmation
                                                              # pa.generation : si oui ou non il fait creer des points d'absence
                                                              # /!\ : ne mettez pas de "_" dans les noms d'spp car biomod utilise les "_" comme des espaces, ne mettez pas des n° non plus. 
...
for (sp in sp_list$sp) {
  ...
  # Plot baseline et P-A data
  plot(baseline[[1]])
  points(sp_occurrence[, c("x", "y")], cex = .5, 
         pch = c(1, 16)[sp_occurrence$Observed + 1]) # le + 1 est pour que les points soient bien colorés (pt noir vs vide) 
