...
for (i in 1:nrow(sp_list))
{
  ...
  {
    PA_table <- readRDS(paste0("data/bg_table_", sp)) # table des runs de background générée avec le script 8. 
    PA_strategy <- "user.defined" # pour dire qu'on a déjà généré les points de background nous même
  ...
  {
    # Pour éviter un bug, je set les objets to NULL pour les espèces avec des abscences
  ...
  run_data <- BIOMOD_FormatingData(
    ...
    # arguments eval.XXXX
    PA.nb.rep = ncol(PA_table), # Nombre de répétitions de background trouvé à partir du nb de colonnes
    PA.strategy = PA_strategy, 
    PA.user.table = PA_table)
  # Attention : si on fait filter.raster = TRUE, on va supprimer les abscences et garder seulement les présences !! Il ne faut pas faire ça.
  ...
  #### Attention : commentaire écrit avant la dernièrte mise à jour de Biomod donc peut-être plus valable
  ...
  # par run
  calib_summary <- summary(run_data, calib.lines =  folds) # On peut ignorer la col "Undefined"
  ...
  for (cvrun in 1:nrow(calib_summary)) {
    ...
    # pour pouvoir compter P vs A pour pouvoir ré-équilibrer mes classes en mettant nos poids manuellement
    ...
    if(sp_list$pa.generation[i] == "no")
    {
      ...
      bgNum <- calib_summary$Pseudo_Absences[cvrun] # depend de la répartition en run et fold
    }
    
    # On a bcp plus de background que de présence. Comment ré-équilibrer les classes ?
    ...
    # Attention : on part du principe que les abscences ou les backgrounds sont supérieures ou égale aux présence mais ça ne marchera pas si les présences sont plus nombreuses que les absences. Il faudra dans ce cas adapter le code en ré-équilibrant. # Peut-être qu'il n'y a pas besoin de corriger la formule finalement, A VERIFIER.  
    ...
    # stepwise, le mieux étant de fitter une régression régularisée (A VERIFIER si possible dans biomod),
    ...
    # Possible d'aller plus loin avec finetunig : caret ou fct de biomod2
    ...
    # Les param par défaut ne sont pas très bons et on ne connait pas dans la lit des bons param --> il faut en tester.
  ...
  model_parameters <- bm_ModelingOptions(
    ...
    user.val = list(     # Paramètres que nous avons défini nous même pour chaque md 
    ...
    bm.format = run_data,
    calib.lines = folds # cf. tableau dans ppt.
  )
  ...
  model_runs <- BIOMOD_Modeling(
    ...
    OPT.strategy = "user.defined", # Quels paramètres pour les modèles ? # erreur A VERIFIER (par Boris)
    OPT.user.val = model_parameters, # Vos paramètres customisés 
    OPT.user.base = "bigboss", # Quels paramètres par défaut pour les modèles
    # pour les paramètres que vous n'avez pas ajustés ?
    
    # Pas clair quelle est la difference entre OPT.strategy et OPT.user.base A VERIFIER
